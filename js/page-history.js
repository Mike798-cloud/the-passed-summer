...
(async()=>{try{
 ...
 const visible=data.filter(x=>{
   ...
   if(x.id==='hist_deleted_reply_cache_2019')return stage>=4&&hasFlag('viewedSamplingProtocol');
   if(x.id==='hist_center_cooperation')return stage>=3;
   if(x.id==='hist_center_service_chain')return stage>=5&&hasFlag('viewedCenterPublic');
   if(x.id==='hist_center_incident_2019'||x.id==='hist_lin_transfer_2022')return stage>=5&&hasFlag('viewedCenterPublic');
