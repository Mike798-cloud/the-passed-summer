...
function shell(){
  ...
  <header class="masthead"><div class="inner"><a href="index.html" aria-label="返回返校服务平台首页"><img class="school-mark" src="assets/img/school-mark.svg" alt="栖岚实验高级中学校徽"></a><div class="school-title"><h1>2026 秋季学期返校服务平台</h1><p>栖岚实验高级中学 · 学生事务与后勤协同服务</p></div><div class="masthead-status"><span class="status-pill soft-anomaly" data-session-status><span class="status-dot"></span><span data-service-status data-anomaly-text data-l2="当前校园会话已登录 · 历史查询同步中" data-l3="当前校园会话已登录 · 历史索引复核" data-l4="当前校园会话已登录 · 学生事务事项处理中">${statusNormal}</span></span><br><span>${accountLine}</span></div></div></header>
  ...
  <footer class="footer"><div class="inner"><div><b>栖岚实验高级中学</b> · 信息中心 / 学生处<br><span class="tiny">返校服务平台 v2026.08 · 校内服务标识 QL-RET-2026 · 数据更新以页面业务时间为准</span><br><span class="tiny">隐私提示：返校服务使用校园统一身份会话，本页面不要求重复填写证件号码或账户密码。</span></div><div class="controls"><button type="button" data-open-settings>阅读设置</button><button type="button" data-print>打印当前页</button></div></div></footer>
  <div class="confirm-layer settings-layer" data-settings-layer hidden role="dialog" aria-modal="true" aria-labelledby="reading-settings-title"><div class="confirm-card settings-card"><h3 id="reading-settings-title">阅读设置</h3><p class="small muted">这些选项只调整当前设备的显示与声音，不修改已提交的返校数据。</p><label class="setting-row"><span><b>减少动态效果</b><small>降低轮播、故障字符与过渡动画的动态表现。</small></span><input type="checkbox" data-setting-motion></label><label class="setting-row"><span><b>静音网站提示音</b><small>关闭铃声与网站反馈提示音。</small></span><input type="checkbox" data-setting-muted></label><label class="setting-row"><span><b>核对字段下划线</b><small>默认关闭。开启后，为表格和业务页面中的部分字段增加普通下划线，方便低对比度屏幕阅读。</small></span><input type="checkbox" data-setting-effective></label><div class="settings-save"><div><b>本机办理记录</b><p class="tiny muted">包含页面访问、搜索历史、返校提交状态与最终处理回执。阅读设置不会随记录重置。</p></div><button type="button" class="btn" data-reset-save>重置记录</button></div><div class="settings-save"><div><b>关于作品</b><p class="tiny muted">完整内容可免费浏览。若愿意，可自愿支持作者继续创作。</p></div><button type="button" class="btn" data-support-author>支持作者 1 元</button></div><div class="confirm-actions"><button type="button" class="btn primary" data-close-settings>完成</button></div></div></div></div>`;
}
...
function bindFooter(){
  ...
  document.querySelector('[data-reset-save]')?.addEventListener('click',()=>{
    ...
    const box=document.createElement('div');box.className='notice-box warn reset-confirm-box';box.innerHTML='<strong>确认重置本机办理记录？</strong><p class="small">将清除页面访问、搜索历史、返校提交状态、最终处理回执与本作自动弹出记录。阅读设置会保留；已完成支持标记不会被删除。</p><div class="actions"><button type="button" class="btn" data-reset-cancel>取消</button><button type="button" class="btn warn" data-reset-confirm>确认重置</button></div>';
...
export function showLoadError(err){pageRoot().innerHTML=`<div class="panel"><h2 class="section-title">页面暂时无法读取</h2><p>当前静态文件需要通过 HTTP 方式打开。请将这些文件部署到 GitHub Pages 或任意静态 HTTP 服务后再访问。</p><p class="small muted">${safeText(err?.message||'未知错误')}</p></div>`}
