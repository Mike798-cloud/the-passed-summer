import {initState,settings,patchSettings,countdownInfo,anomalyLevel,storageName,resetAll,flag,hasFlag,finalChoice,simulatedNow,currentReturnSubmitted,plotStage,environmentSnapshot,atmosphereStage} from './state.js';
...
function shell(){
  ...
  const statusNormal=erased?'校园会话已失效':'当前校园会话已登录';
...
function updateClock(){
  const c=countdownInfo(),env=environmentSnapshot(),now=env.now;
  ...
  if(lastAtmosphere!==env.stage){
    ...
    if(env.stage>=6&&!hasFlag('navSlipSeen')&&!settings().reducedMotion){
      flag('navSlipSeen');
    ...
    if(env.stage>=4&&!hasFlag('ambientEveningCueShown')){flag('ambientEveningCueShown');setTimeout(()=>playCue('soft',.11),950)}
    if(env.stage>=7&&!hasFlag('ambientLateCueShown')){flag('ambientLateCueShown');setTimeout(()=>playCue('radio',.08),1350)}
  }
}

shell();syncSettings();bindFooter();applyAnomalyFrame();updateClock();setInterval(updateClock,30000);setTimeout(()=>maybeAutoSupport(),1250);
window.addEventListener('pageshow',()=>{applyAnomalyFrame();syncSettings();updateClock()});window.addEventListener('ql:state',()=>{applyAnomalyFrame();updateClock()});
...
export function sideCommon(){const submitted=currentReturnSubmitted(),erased=!!finalChoice(),stage=plotStage(),env=environmentSnapshot();const st=erased?'账户已转出':submitted?'已提交':'待提交';return `<aside class="sidebar"><div class="card"><h3 class="section-title">办理状态</h3><div class="small">2026 返校信息：<b>${st}</b></div><div class="small">截止时间：8 月 31 日 24:00</div><div class="deadline" data-deadline></div></div><div class="card current-campus-mini"><h3 class="section-title">当前校园</h3><div><span>天气</span><b>${env.weather}</b></div><div><span>校园广场在线</span><b>${env.online}</b></div><div><span>服务时段</span><b>${env.service}</b></div><p class="tiny muted">${env.note}</p></div><div class="card quick-links"><h3 class="section-title">常用服务</h3><a href="my-return.html">核对我的返校信息</a><a href="square.html">校园广场</a><a href="history.html">公开校史与归档</a><a href="notices.html?year=2026">2026 返校通知</a></div><div class="card"><h3 class="section-title">系统提示</h3><p class="small soft-anomaly" data-anomaly-text data-l2="历史公开资料可能来自多个旧系统，检索结果以归档页为准。" data-l3="部分连续历史查询会进入访问质量复核，用于排查旧索引兼容问题。" data-l4="当前访问质量复核已进入学生事务服务流转，请留意会话事项。">${stage? '历史公开资料来自多个旧平台，显示范围可能因迁移状态不同而变化。':'返校信息提交后可继续查看住宿、交通、教材和校园公共信息。'}</p><p class="tiny muted">存储模式：${storageName()==='localStorage'?'本机断点续办':storageName()==='sessionStorage'?'会话临时保存':'当前页面会话'}</p></div></aside>`}
